...
export function formatPr(pr: PrJson): string {
...
export function formatPrList(prs: PrJson[]): string {
...
export function formatPrChecks(checks: Array<{ name: string; state: string; description?: string }>, prNumber: number): string {
  if (checks.length === 0) return `No checks for PR #${prNumber}`;
