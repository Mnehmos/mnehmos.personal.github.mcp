 ...
 */
export function formatActiveContext(ctx: ActiveContext): string {
  const lines: string[] = [];
  ...
  if (ctx.inProgress.length > 0) {
    ...
    ctx.inProgress.forEach((task) => {
      ...
      lines.push(
        `      Assignee: ${task.assignee} | Started: ${task.startedAt}`
      );
    });
  }
  ...
  if (ctx.milestones.length > 0) {
    ...
    ctx.milestones.forEach((m) => {
      ...
      lines.push(
        `   ${m.title}: ${bar} ${pct}% (${m.progress.closed}/${total})${due}`
      );
    });
  }
    ...
    ctx.pullRequests.forEach((pr) => {
      const statusEmoji =
        pr.reviewStatus === "APPROVED"
          ...
          : "\u{1F7E1}";
      lines.push(
        `   ${statusEmoji} #${pr.number} ${truncate(pr.title, 40)} (@${
          pr.author
        })`
      );
    });
 ...
 */
export function formatContextSummary(ctx: ActiveContext): string {
  const parts: string[] = [];
  ...
  if (ctx.blockers.length > 0) {
    parts.push(
      `Blockers: ${ctx.blockers.map((b) => `#${b.number}`).join(", ")}`
    );
  }

  if (ctx.inProgress.length > 0) {
    parts.push(
      `In Progress: ${ctx.inProgress.map((t) => `#${t.number}`).join(", ")}`
    );
  }
 ...
 */
export function formatBlockers(blockers: ActiveContext["blockers"]): string {
  if (blockers.length === 0) {
    return "\u2705 No blockers!";
  }

  const lines: string[] = ["\u{1F6A8} BLOCKERS REQUIRING ATTENTION:"];
