...
// Meta tool handler
async function handleMetaTool(input: { action?: string; title?: string; body?: string; labels?: string }): Promise<{ success: boolean; formatted?: string; error?: string; data?: any }> {
      ...
      inputSchema: {
        ...
        properties: Object.fromEntries(
          Object.entries(tool.inputSchema.shape).map(([key, value]) => [
            key,
            { type: 'string', description: (value as any).description },
          ])
        ),
      },
    })),
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    ...
    switch (name) {
      // Phase 1
      case 'github_issue':
        result = await handleIssueTool(args as any);
        break;
      case 'github_context':
        result = await handleContextTool(args as any);
        break;
      case 'github_project':
        result = await handleProjectTool(args as any);
        break;
      case 'github_wiki':
        result = await handleWikiTool(args as any);
        break;
      case 'github_links':
        result = await handleLinksTool(args as any);
        break;

      // Phase 2
      case 'github_pr':
        result = await handlePrTool(args as any);
        break;
      case 'github_label':
        result = await handleLabelTool(args as any);
        break;
      case 'github_milestone':
        result = await handleMilestoneTool(args as any);
        break;
      case 'github_search':
        result = await handleSearchTool(args as any);
        break;

      // Phase 3
      case 'github_notification':
        result = await handleNotificationTool(args as any);
        break;
      case 'github_reaction':
        result = await handleReactionTool(args as any);
        break;
      case 'github_status':
      ...
      // Meta
      case 'github_meta':
        result = await handleMetaTool(args as any);
        break;
