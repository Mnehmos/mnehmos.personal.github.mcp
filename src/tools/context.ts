...
import { ghIssue, ghPr, ghApi, type IssueJson } from '../cli.js';
import { GithubContextInputSchema, type GithubContextInput, type ActiveContext } from '../schemas/context.js';
import { formatActiveContext, formatBlockers, formatContextSummary } from '../formatters/context.js';
 ...
 */
export async function handleContextTool(input: GithubContextInput): Promise<ContextToolResult> {
...
async function handleGetContext(input: GithubContextInput): Promise<ContextToolResult> {
  ...
  if (include.includes('recent_comments')) {
    ...
    if (result.success && result.data) {
      ...
      const comments: ActiveContext['recentComments'] = [];
...
async function handleSetFocus(input: GithubContextInput): Promise<ContextToolResult> {
  if (!input.issue) {
...
async function handleGetBlockers(input: GithubContextInput): Promise<ContextToolResult> {
  const result = await ghIssue.list(input.repo, {
    state: 'open',
    labels: ['blocker'],
    limit: 50,
...
async function handleGetTimeline(input: GithubContextInput): Promise<ContextToolResult> {
...
// Helpers
function detectPriority(labels: Array<{ name: string }>): 'critical' | 'high' | 'medium' | 'low' | 'none' {
...
function extractBlockingIssues(body: string): number[] {
