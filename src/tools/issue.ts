...
import { GithubIssueInputSchema, normalizeLabel, normalizeState, type GithubIssueInput } from '../schemas/issue.js';
 ...
 */
export async function handleIssueTool(input: GithubIssueInput): Promise<IssueToolResult> {
  // Validate input
...
async function handleCreate(input: GithubIssueInput): Promise<IssueToolResult> {
  if (!input.title) {
...
async function handleGet(input: GithubIssueInput): Promise<IssueToolResult> {
  if (!input.issue) {
...
async function handleUpdate(input: GithubIssueInput): Promise<IssueToolResult> {
  if (!input.issue) {
...
async function handleClose(input: GithubIssueInput): Promise<IssueToolResult> {
  if (!input.issue) {
    return { success: false, action: 'close', error: 'Issue number is required' };
  }

  const result = await ghIssue.close(
    input.repo,
    input.issue,
    input.reason,
    input.comment
...
async function handleReopen(input: GithubIssueInput): Promise<IssueToolResult> {
  if (!input.issue) {
...
async function handleComment(input: GithubIssueInput): Promise<IssueToolResult> {
  if (!input.issue) {
...
async function handleSearch(input: GithubIssueInput): Promise<IssueToolResult> {
  if (!input.query) {
...
async function handleList(input: GithubIssueInput): Promise<IssueToolResult> {
...
async function handleBatchUpdate(input: GithubIssueInput): Promise<IssueToolResult> {
  if (!input.operations || input.operations.length === 0) {
    return { success: false, action: 'batch_update', error: 'Operations array is required' };
  }

  const results: Array<{ issue: number; success: boolean; error?: string }> = [];
