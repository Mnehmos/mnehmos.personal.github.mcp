...
function getType(labels) {
    ...
    if (labelNames.some(l => l.includes('idea')))
        return { name: 'Idea', emoji: '\u{1F4A1}' };
...
export function formatIssue(issue, options = {}) {
    const { includeBody = true, includeComments = true, maxBodyLines = 10, maxComments = 3, } = options;
 ...
 */
export function formatSearchResults(issues, query) {
    if (issues.length === 0) {
