...
export const IssueSchema = z.object({
    ...
    state: z.enum(['open', 'closed', 'OPEN', 'CLOSED']).transform(s => s.toLowerCase()),
...
export const GithubIssueInputSchema = z.object({
    ...
    labels: z.preprocess((val) => typeof val === 'string' ? val.split(',').map(s => s.trim()).filter(Boolean) : val, z.array(z.string()).optional()).describe('Labels to apply (fuzzy matched)'),
    assignees: z.preprocess((val) => typeof val === 'string' ? val.split(',').map(s => s.trim()).filter(Boolean) : val, z.array(z.string()).optional()).describe('Assignees (@me for self)'),
