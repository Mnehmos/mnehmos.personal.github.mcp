...
export async function handleStatusTool(input) {
...
async function handleGet(input) {
    ...
    const args = ['status'];
    if (input.org)
        args.push('-o', input.org);
    if (input.exclude?.length)
        input.exclude.forEach(e => args.push('-e', e));
...
async function handleAssigned(input) {
    ...
    const [issues, prs] = await Promise.all([gh(issueArgs), gh(prArgs)]);
