 ...
 */
export async function handleNotificationTool(input) {
...
async function handleList(input) {
    ...
    if (input.before)
        params.append('before', input.before);
    ...
    const result = await ghApi.rest(`${endpoint}?${params.toString()}`);
    if (!result.success) {
...
async function handleGet(input) {
    ...
    const result = await ghApi.rest(`/notifications/threads/${input.threadId}`);
    if (!result.success) {
...
async function handleMarkRead(input) {
    if (!input.threadId) {
        return { success: false, action: 'mark_read', error: 'Thread ID is required' };
    }
    const result = await ghApi.rest(`/notifications/threads/${input.threadId}`, 'PATCH');
    if (!result.success) {
...
async function handleMarkAllRead(input) {
    let endpoint = '/notifications';
    if (input.repo) {
        const [owner, name] = input.repo.split('/');
        endpoint = `/repos/${owner}/${name}/notifications`;
    }
    const result = await ghApi.rest(endpoint, 'PUT', {
...
async function handleMarkDone(input) {
    if (!input.threadId) {
        return { success: false, action: 'mark_done', error: 'Thread ID is required' };
    }
    const result = await ghApi.rest(`/notifications/threads/${input.threadId}`, 'DELETE');
    if (!result.success) {
...
async function handleSubscribe(input) {
    ...
    const result = await ghApi.rest(`/notifications/threads/${input.threadId}/subscription`, 'PUT', { subscribed: true });
    if (!result.success) {
...
async function handleUnsubscribe(input) {
    if (!input.threadId) {
        return { success: false, action: 'unsubscribe', error: 'Thread ID is required' };
    }
    const result = await ghApi.rest(`/notifications/threads/${input.threadId}/subscription`, 'DELETE');
    if (!result.success) {
