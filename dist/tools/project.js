 ...
 */
import { ghProject, gh } from '../cli.js';
 ...
 */
export async function handleProjectTool(input) {
...
async function handleGetProject(input) {
    ...
    if (isNaN(projectNum)) {
        ...
        const found = listResult.data.find(p => p.title.toLowerCase() === String(input.project).toLowerCase());
        if (!found) {
...
async function handleGetProjectByNumber(owner, projectNumber, title) {
    ...
    for (const item of items) {
        ...
        columnMap.get(status).items.push({
            ...
            assignees: item.content?.assignees?.map((a) => a.login) || [],
            labels: item.content?.labels?.map((l) => l.name) || [],
        });
    }
    const project = {
...
async function handleGetColumn(input) {
    ...
    const column = project.columns.find(c => c.name.toLowerCase() === input.column.toLowerCase());
    if (!column) {
        return {
            success: false,
            action: 'get_column',
...
async function getProjectItemId(owner, repo, projectNumber, issueNumber) {
    // Get project items and find the one matching this issue
    const result = await ghProject.items(owner, projectNumber);
    if (!result.success || !result.data?.items)
        return null;
    const item = result.data.items.find((i) => i.content?.number === issueNumber);
    return item?.id || null;
}
async function handleListFields(input) {
    if (!input.project) {
...
async function handleSetField(input) {
    ...
    if (!result.success) {
        ...
        const retryResult = await gh(retryArgs);
...
async function handleMoveItem(input) {
