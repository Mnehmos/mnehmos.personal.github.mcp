 ...
 */
export async function handleLinksTool(input) {
...
async function handleAddLink(input) {
    ...
    const result = await ghIssue.comment(input.repo, input.source, `\u{1F517} ${linkText}`);
    if (!result.success) {
...
async function handleRemoveLink(input) {
...
async function handleGetGraph(input) {
    if (!input.issue) {
    ...
    while (queue.length > 0) {
        ...
        if (visited.has(issue) || currentDepth > depth)
            continue;
        visited.add(issue);
        // Fetch issue
...
async function handleFindBlockers(input) {
    if (!input.issue) {
...
async function handleFindCycles(input) {
...
// Helpers
function formatLinkText(type, target) {
    switch (type) {
...
function formatDependencyGraph(graph, rootIssue) {
    const lines = [];
